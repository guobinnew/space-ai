...
function getToolSummary(toolName: string, input: Record<string, unknown>): string {
  ...
  switch (toolName) {
    ...
    case 'Grep':
      return typeof obj.pattern === 'string' ? obj.pattern : '';
    case 'TodoWrite':
      return Array.isArray(obj.todos) ? `${obj.todos.length} tasks` : '';
    case 'WebFetch':
      return typeof obj.url === 'string' ? obj.url : '';
    case 'NotebookEdit':
      return typeof obj.notebook_path === 'string' ? obj.notebook_path : '';
    case 'WebSearch':
      return typeof obj.query === 'string' ? obj.query : '';
    case 'AskUserQuestion':
      return Array.isArray(obj.questions) ? `${obj.questions.length} questions` : '';
    case 'EnterPlanMode':
      return '';
    case 'ExitPlanMode':
