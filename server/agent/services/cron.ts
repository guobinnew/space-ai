...
export function cronToHuman(cron: string): string {
  ...
  if (minute.match(/^\d+$/) && hour === '*' && dayOfMonth === '*' && month === '*' && dayOfWeek === '*') {
    ...
    if (m === 0) return '每小时'
    return `每小时 :${m.toString().padStart(2, '0')}`
  }
  ...
  if (dayOfMonth === '*' && month === '*' && dayOfWeek === '*') {
    return `每天 ${formatLocalTime(m, h)}`
  }

  if (dayOfMonth === '*' && month === '*' && dayOfWeek.match(/^\d$/)) {
    const dayIndex = parseInt(dayOfWeek, 10) % 7
    return `每 ${DAY_NAMES[dayIndex]} ${formatLocalTime(m, h)}`
  }

  if (dayOfMonth === '*' && month === '*' && dayOfWeek === '1-5') {
    return `工作日 ${formatLocalTime(m, h)}`
  }
